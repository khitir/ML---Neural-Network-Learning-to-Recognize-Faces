...
def load_images_and_labels(folder, target_size=(128, 128)):
    ...
    for root, dirs, files in os.walk(folder):
        for file in files:
            if file.endswith('.pgm') and 'sunglasses' not in file:
...
# Display the test image in higher quality
plt.figure(figsize=(6, 6))  # Increase the figure size
plt.imshow(test_image_array.squeeze(), cmap='gray', interpolation='nearest')  # Adjust interpolation
plt.title('Actual Emotion: angry')  # Replace 'happy' with the actual emotion of the image
